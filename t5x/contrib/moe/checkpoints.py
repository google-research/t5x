...
class UpcycleCheckpointer(checkpoints.Checkpointer):
  ...
  def __init__(self,
    ...
    Args:
      ...
      use_gda: If True, enabled gda_lib.GlobalDeviceArray. GDA is not yet
        supported for sparse upcycling.
    ...
    """
    if use_gda:
      # TODO(jamesleethorp): Support sparse upcycling under GDA.
      raise ValueError('Sparse upcycling under GDA is not yet supported. '
                       'Please set `use_gda`=False.')
...
async def _read_upcycle_slice_from_ts(compressed_arr, param_info):
  """Reads array from tensorstore and broadcasts to experts."""
...
async def _read_upcycle_ts(param_info: _ParameterInfo,
  ...
  def read_upcycle_slice_from_np(compressed_arr):
  ...
  if isinstance(maybe_ts_spec, np.ndarray) and param_info:
    # TODO(jamesleethorp): Support sparse upcycling under GDA. Should pass:
    #  if mesh is not None and axes is not None: return maybe_ts_spec
    if param_info.local_chunk_info:
      arr = maybe_ts_spec
      if (not m_or_v) and is_expert_param:
        return read_upcycle_slice_from_np(arr)
      else:
        arr = maybe_ts_spec
  ...
  if mesh is None or axes is None:
    ...
    if (not m_or_v) and is_expert_param and info:
      arr = await _read_upcycle_slice_from_ts(t, param_info)
    else:
