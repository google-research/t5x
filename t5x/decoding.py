...
class LogitCallbackFnCallable(typing_extensions.Protocol):
  ...
  def __call__(self,
               ...
               arg: Optional[str] = None) -> jnp.ndarray:
    ...
    Args:
      logits: array: [vocab] int32 logits for the current decoding step.
      state: SamplingLoopState: The current loop state.
      arg: string: an optional argument useful when passed dynamically to
        provide context from the request to the callback.

    Returns:
      logits: array: [vocab] int32 modified logits.
    """
    ...
...
def temperature_sample(
    ...
    logit_callback_arg: Optional[str] = None,
  ...
  Args:
    ...
    logit_callback_arg: string: an optional argument that will be passed to
